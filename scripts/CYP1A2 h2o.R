...
calc_aucpr <- function(df_iter) {
  ...
  # True labels as 1/0
  truth <- ifelse(df_iter$outcome == "Include", 1, 0)
...
dfPICOSfinal <- dfwithPICOS %>%
  ...
  mutate(reviewn = case_when(review =='Yes'~ 0,   ### create numerical variables with review 
         ...
         Sn = case_when(S =='Yes'~ 1,
                        S == 'No' ~ 0,
                        S == 'Uncertain' ~ 0.5)) %>%
  dplyr::select(key, keywords, title, abstract, authors, year, review, P, I, C, O, S,reviewn, Pn, In,Cn, On, Sn, search1, search2, search3) %>%
...
saveRDS(results, 'results/resultsCYP.rds')

##summarise#####
results <- read_rds('results/resultsCYP.rds')
EXCINC <- results %>% 
  group_by(configs) %>%
  filter(outcome == "Include" & newpred < thresh) %>%
...
sumtextPICOS <- results %>%
  ...
  merge(.,results %>%
          group_by(new, outcome, .drop= F) %>%
          summarise(
            foundft = (sum(baked_df$outcome == "Include") - sum(outcome == "Include")) /
              sum(baked_df$outcome == "Include") * 100)    %>%
          filter(outcome == "Include") %>%
          select(-outcome))%>%
  mutate(recall = inc.correct * 100 / (inc.correct + exc.incorrect) ,
...
ggplot(data = calclong, aes(x = percread, y = value)) +
  ...
  geom_vline(xintercept = 3.717472, linetype = 2, colour = 'orange')+
  geom_vline(xintercept = 7.311029, linetype = 3, colour = 'gray20')+
  theme_classic() 
