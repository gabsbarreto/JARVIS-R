...
dfPICOSfinal <- read_rds('dfGPT.antidepr.complete3.rds') %>%
  ...
  mutate(screening = ifelse(screening == "Yes", 'Include', 'Exclude'),
  FTscreening = ifelse(is.na(FTscreening),'Exclude', FTscreening )) %>%
...
dftoken <- dfPICOSfinal %>%
  ...
  filter(!str_detect(abstract, regex("no abstract", ignore_case = TRUE)), nchar(abstract) >= 100) %>%
...
saveRDS(baked_df, 'Philippa 4 final.rds')
##START FROM HERE WITH THE BAKED READY####
baked_df <- read_rds('Philippa 4 final.rds') %>%
  mutate(weightsc = ifelse(screening == "Include", 20,1),
         FTscreening = as.factor(FTscreening))
...
each5.2 <- function(df1, max,  epoc, hidu, activ, stop_rounds, stop_tol, rates_anneal,min_batch, l2, rate) {
  ...
  for (i in seq_len(max)) {
    ...
    baked_df[complete.cases(baked_df) == F,]
...
results <- run_each5_with_repeats(baked_df, 100,  epochs,hiddenunis,activ,  stop_rounds, stop_tol,rates_anneal, min_batch,l2,rate, TIMES)
saveRDS(results, 'resultsphil4.rds')
###summarise#####
results <- read_rds('resultsphil4.rds')
EXCINC <- results %>% 
  group_by(configs) %>%
...
ggplot(data = subset(results, new > 0 & new < 11), aes(x = (newpred))) + 
...
ggplot(data = subset(results, new > 40 & new < 51), aes(x = (newpred))) + 
  #geom_density(alpha= 0.2, aes(colour = RELEVANCE)) +
  geom_histogram( aes(fill = FTscreening,y = after_stat(density))) +
  scale_fill_manual(values = c('black', 'red')) +
  geom_vline(aes(xintercept = thresh), colour = 'blue', linetype = 2) +
